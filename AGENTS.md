...
- `apps/desktop` is the Electron + Vite + React desktop app shipped as Assetwell.
- `apps/desktop/src` is the renderer; `apps/desktop/electron` is the Electron Host and owns native APIs, IPC, and CLI spawning.
- `packages/desktop-bridge` is the shared typed `window.assetwell` contract; update it with preload, IPC handlers, and renderer usage together.
...
- Assetwell wraps pinned `@higgsfield/cli` from `apps/desktop`; the Electron Host prefers the vendored native `hf` executable and falls back to global `higgsfield` only for recovery.
...
- Renderer access is via `window.assetwell` exposed in `apps/desktop/electron/preload.ts`; keep `contextIsolation: true` and `nodeIntegration: false` assumptions intact.
...
- Assetwell does not own Higgsfield auth, accounts, workspaces, models, uploads, or generation jobs; it wraps the CLI and presents host-owned status/output.
